...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        QFuture<void> future1 = QtConcurrent::run([&](){concurRace1->DoWork(&number,true,ui->sb_initNum->value());});
        QFuture<void> future2 = QtConcurrent::run([&](){concurRace2->DoWork(&number,true,ui->sb_initNum->value());});
    }
